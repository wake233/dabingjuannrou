...
// --- Tests for woodcut/ink with entity-specific shapes (v4 renderers) ---
test("木刻渲染器复用实体形状骨架并应用高反差块面与方向性刻线", () => {
  ...
  // Multiple fill-bearing elements from the entity skeleton each get tone blocks (>= 4)
  assert.ok(rendered.children.filter(child => child.getAttribute("data-tone-block")).length >= 4,
    `应有至少4个色块元素，实际 ${rendered.children.filter(child => child.getAttribute("data-tone-block")).length}`);
  // Carve lines exist — direction depends on grammar category (figure → horizontal)
  const carveLines = rendered.children.filter(child => child.getAttribute("data-carved-line"));
  assert.ok(carveLines.length >= 6,
    `应有至少6条刻线，实际 ${carveLines.length}`);
  // Person is "figure" category → default carve direction is "horizontal"
  const horizontalLines = carveLines.filter(c => c.getAttribute("data-carved-line") === "horizontal");
  assert.ok(horizontalLines.length >= 6,
    `人物类应有水平刻线，实际 ${horizontalLines.length}`);
  assert.match(rendered.getAttribute("transform"), /scale\(-1 1\)/);
  // Must differ from storybook rendering
...
test("水墨渲染器使用浓淡层次、飞白和受控墨迹并保持实体整体", () => {
  ...
  // Fill-bearing elements from entity skeleton + wash-layer ellipses each get ink-wash labels
  assert.ok(rendered.children.filter(child => child.getAttribute("data-ink-wash")).length >= 3,
    `应有至少3个墨层元素，实际 ${rendered.children.filter(child => child.getAttribute("data-ink-wash")).length}`);
  // Nature entities get flying-white strokes (>= 2)
  assert.ok(rendered.children.filter(child => child.getAttribute("data-flying-white") === "true").length >= 2);
  // Controlled ink specks: nature default = 3
  assert.equal(rendered.children.filter(child => child.getAttribute("data-ink-speck") === "controlled").length, 3);
  // Must differ from woodcut rendering
  assert.notEqual(rendered.children.length, renderArtworkEntity(entity, "woodcut").children.length);
});

test("木刻风格的人物实体可识别为人形（有头、身体、四肢等形状元素）", () => {
  const person = { id: "wc-person", kind: "entity", templateId: "person", name: "人", x: 0, y: 0,
    width: 200, height: 350, rotation: 0, opacity: 1, params: {} };
  const cat = { id: "wc-cat", kind: "entity", templateId: "cat", name: "猫", x: 0, y: 0,
    width: 180, height: 150, rotation: 0, opacity: 1, params: {} };
  const mountain = { id: "wc-mountain", kind: "entity", templateId: "mountain", name: "山", x: 0, y: 0,
    width: 500, height: 300, rotation: 0, opacity: 1, params: {} };

  const wp = renderArtworkEntity(person, "woodcut");
  const wc = renderArtworkEntity(cat, "woodcut");
  const wm = renderArtworkEntity(mountain, "woodcut");

  // Different entity types produce different shapes
  assert.notEqual(wp.children.length, wc.children.length,
    "人物木刻应与猫木刻元素数量不同");
  assert.notEqual(wc.children.length, wm.children.length,
    "猫木刻应与山木刻元素数量不同");

  // Person contains an ellipse (head) in woodcut mode
  const personEllipses = [...wp.children].filter(c => c.tagName === "ellipse");
  assert.ok(personEllipses.length >= 1, "人物木刻应包含头部椭圆");

  // Mountain contains multiple triangular paths (peaks)
  const mountainToneBlocks = [...wm.children].filter(c => c.getAttribute("data-tone-block"));
  assert.ok(mountainToneBlocks.length >= 2, "山木刻应有多个色块（多峰结构）");

  // All use woodcut palette
  const allFills = [...wp.children, ...wc.children, ...wm.children]
    .map(c => c.getAttribute("fill"))
    .filter(f => f && f !== "none");
  assert.ok(allFills.length > 0);
  for (const fill of allFills) {
    assert.ok(/^#[0-9a-f]{6}$/i.test(fill), `填充应为六位十六进制色: ${fill}`);
  }
});

test("水墨风格的树实体可识别为树形（有树干、树冠等）", () => {
  const tree = { id: "ink-tree", kind: "entity", templateId: "tree", name: "树", x: 0, y: 0,
    width: 200, height: 280, rotation: 0, opacity: 1, params: {} };
  const person = { id: "ink-person", kind: "entity", templateId: "person", name: "人", x: 0, y: 0,
    width: 180, height: 320, rotation: 0, opacity: 1, params: {} };

  const it = renderArtworkEntity(tree, "ink");
  const ip = renderArtworkEntity(person, "ink");

  // Tree and person should have different shapes (not generic ellipses)
  assert.notEqual(it.children.length, ip.children.length,
    "树木墨应与人物墨元素数量不同");

  // Tree should have circular canopy elements + trunk (from template)
  const treeChildren = [...it.children];
  assert.ok(treeChildren.length >= 4, `树应有足够子元素，实际 ${treeChildren.length}`);

  // Person should have more elements than tree (character detail)
  assert.ok(ip.children.length > it.children.length,
    `人物墨(${ip.children.length})应比树墨(${it.children.length})有更多元素`);

  // Ink colors are within ink palette range (dark greys)
  const inkFills = [...it.children]
    .map(c => c.getAttribute("fill"))
    .filter(f => f && f !== "none" && !f.startsWith("url("));
  for (const fill of inkFills) {
    assert.ok(/^#[0-9a-f]{6}$/i.test(fill), `墨色应为六位十六进制: ${fill}`);
  }
});

test("renderArtworkEntity 读取并使用 _grammarCategory, _blockCount 等元数据", () => {
  // Entity with explicit art-engine metadata
  const entity = {
    id: "meta-test", kind: "entity", templateId: "cat", name: "猫", x: 0, y: 0,
    width: 180, height: 150, rotation: 0, opacity: 1, params: {},
    _grammarCategory: "figure", _blockType: "high-contrast",
    _blockCount: 5, _carveDirection: "horizontal",
    _accentBlock: true
  };

  const rendered = renderArtworkEntity(entity, "woodcut");

  // With blockCount=5, the carve lines cycle through 5 tones (more lines)
  const carveLines = rendered.children.filter(
    child => child.getAttribute("data-carved-line") === "horizontal"
  );
  assert.ok(carveLines.length >= 5,
    `水平刻线应至少5条，实际 ${carveLines.length}`);

  // Accent block should be present (figure)
  const accents = rendered.children.filter(
    child => child.getAttribute("data-accent") === "true"
  );
  assert.equal(accents.length, 1, "figure 类应有强调色块");

  // Ink test with explicit metadata
  const inkEntity = {
    id: "ink-meta", kind: "entity", templateId: "tree", name: "树", x: 0, y: 0,
    width: 200, height: 280, rotation: 0, opacity: 1, params: {},
    _grammarCategory: "nature", _blockType: "ink-wash-gradient",
    _washLayers: 3, _flyingWhite: true, _speckCount: 5
  };

  const inkRendered = renderArtworkEntity(inkEntity, "ink");

  // Should have 3 wash layers + fill elements
  const washElements = inkRendered.children.filter(
    child => child.getAttribute("data-ink-wash")
  );
  assert.ok(washElements.length >= 4,
    `应有至少4个墨层元素(3洗层+填充元素)，实际 ${washElements.length}`);

  // 5 specks as specified
  const specks = inkRendered.children.filter(
    child => child.getAttribute("data-ink-speck") === "controlled"
  );
  assert.equal(specks.length, 5, `应精确5个墨点，实际 ${specks.length}`);

  // Flying white
  const fw = inkRendered.children.filter(
    child => child.getAttribute("data-flying-white") === "true"
  );
  assert.ok(fw.length >= 2, `应有飞白笔触，实际 ${fw.length}`);
});

test("storybook 渲染输出不受木刻/水墨渲染器改动影响", () => {
  const entity = { id: "sb-test", kind: "entity", templateId: "person", name: "人物", x: 0, y: 0,
    width: 240, height: 400, rotation: 0, opacity: 1, params: { variant: "woman", pose: "walking" } };

  const rendered = renderArtworkEntity(entity, "storybook", { quality: "full", namespace: "test" });

  // Storybook should still have its layered style
  assert.equal(rendered.getAttribute("data-renderer"), "storybook");
  assert.equal(rendered.getAttribute("data-art-style"), "storybook-layered");
  assert.equal(rendered.getAttribute("data-semantic-entity"), "person");

  // Should have gradient defs (full quality)
  const hasDefs = [...rendered.children].some(c => c.tagName === "defs");
  assert.ok(hasDefs, "绘本全质量渲染应包含渐变定义");

  // Should have substantial detail
  assert.ok(rendered.children.length >= 8,
    `绘本人物应有充足细节，实际 ${rendered.children.length}`);
});
