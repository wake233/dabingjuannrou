...
| S2-01 | 绘本实体支持参数化部件图，人物及主要环境实体具备造型变体 | renderer 确定性测试、视觉验收 | 部件不成为顶层对象；人物姿态/表情/服装及环境轮廓变化可见 | 待处理 | 待补 |
| S2-02 | 绘本独立渲染器实现柔和轮廓、层叠前中后景与叙事细节 | SVG 结构测试、浏览器截图 | 输出确定、可编辑、与其他风格不只是换色 | 待处理 | 待补 |
| S2-03 | 方案混合、审美精修与锁定约束在正式画布闭环 | Node/app 测试、浏览器操作 | 混合与多轮精修实质改善作品；锁定约束始终成立 | 待处理 | 待补 |
| S2-04 | 云端纹理生成、IndexedDB 缓存、恢复、显式重生成与导出固化闭环 | Python/Node 测试、失败注入 | 选稿后自动请求；普通编辑不重生；缓存缺失回退；失败不阻塞编辑/撤销/恢复/导出 | 待处理 | 待补 |
| S2-05 | 纹理接口严格拒绝非法格式、超限、外部 URL、SVG 与任意属性 | Python 单元测试、代码检查 | 所有非法载荷拒绝；只返回受控栅格纹理和模型元数据 | 待处理 | 待补 |
| S3-01 | 木刻渲染器具有高对比明暗块、方向性刻线、有限色板和独立形状节奏 | SVG 结构测试、视觉验收 | 与绘本结构明显不同；输出确定且可编辑 | 待处理 | 待补 |
| S3-02 | 木刻小稿使用独立构图与负空间策略，风格切换保持叙事关系 | Node 测试、作品集比较 | 同语义场景切换后实体关系不变，构图策略符合木刻特性 | 待处理 | 待补 |
| S4-01 | 水墨渲染器实现浓淡层次、留白、飞白和受控墨迹纹理 | SVG 结构测试、视觉验收 | 与另两种风格结构明显不同；输出确定且可编辑 | 待处理 | 待补 |
| S4-02 | 水墨小稿调整空间、节奏和焦点，纹理不破坏主体可编辑性 | Node/app 测试、作品集比较 | 主体仍可独立选择修改；留白与焦点策略可验证 | 待处理 | 待补 |
