...
| 测试用例 | Agent 输出 | 结果 |
|---------|-----------|------|
| "画一个红色的圆在正中间" | `[{"type":"draw_circle","params":{"cx":500,"cy":500,"r":200,"color":"red"}}]` | ✅ |
| "在左上角画一个蓝色的矩形，然后清空画布，再在右下角画一条绿色的线" | 3条指令依次执行：矩形→清空→画线 | ✅ |
| "画一个紫色三角形，顶点分别在(500,100)、(300,700)和(700,700)，填充为黄色" | `draw_triangle` 含所有顶点坐标和 fill_color | ✅ |
