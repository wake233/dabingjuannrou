...
实体动作只允许 entity_create、entity_update、scene_update；艺术动作只允许 creative 和 texture；一次结果最多 20 个动作或 20 个实体。
creative 仅允许 generate_drafts/select_draft/mix_drafts/refine/lock/unlock/set_style/regenerate_texture；风格仅允许 storybook/woodcut/ink。
texture 仅允许 pending/apply/remove/missing/failed，且只能引用受控 PNG 缓存元数据。
